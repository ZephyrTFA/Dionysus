...
$(curl --fail -H "User-Agent: dionysus/1.0 CI Script" "http://www.byond.com/download/build/$BYOND_MAJOR/$BYOND_MAJOR.${BYOND_MINOR}_byond.zip" -o C:/byond.zip)
...
curl --fail -H "User-Agent: dionysus/1.0 CI Script" ${FALLBACK_URL} -o C:/byond.zip
else
