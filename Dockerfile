...
RUN . ./dependencies.sh \
    && curl -H "User-Agent: dionysus/1.0 CI Script" "http://www.byond.com/download/build/${BYOND_MAJOR}/${BYOND_MAJOR}.${BYOND_MINOR}_byond_linux.zip" -o byond.zip \
    && unzip byond.zip \
    && cd byond \
...
ENTRYPOINT [ "DreamDaemon", "dionysus.dmb", "-port", "1337", "-trusted", "-close", "-verbose" ]
EXPOSE 1337
