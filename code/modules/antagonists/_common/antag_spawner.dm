...
/obj/item/antag_spawner/contract/proc/poll_for_student(mob/living/carbon/human/teacher, apprentice_school)
	...
	polling = TRUE
	var/list/candidates = poll_candidates_for_mob("Do you want to play as a wizard's [apprentice_school] apprentice?", ROLE_SPACE_WIZARD, ROLE_SPACE_WIZARD, 15 SECONDS, src)
	polling = FALSE
...
/obj/item/antag_spawner/slaughter_demon/attack_self(mob/user)
	...
	if(used)
		return
	var/list/candidates = poll_candidates_for_mob("Do you want to play as a [initial(demon_type.name)]?", ROLE_XENOMORPH, ROLE_XENOMORPH, 5 SECONDS, src)
	if(LAZYLEN(candidates))
