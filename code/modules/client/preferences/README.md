...
3. `savefile_identifier` - Whether or not this is a character specific preference (`PREFERENCE_SAVEFILE_CHARACTER`) or one that affects the player (`PREFERENCE_SAVEFILE_PLAYER`). As an example: hair color is `PREFERENCE_SAVEFILE_CHARACTER` while your UI settings are `PREFERENCE_SAVEFILE_PLAYER`, since they do not change between characters.
...
If your preference is `PREFERENCE_SAVEFILE_CHARACTER`, it MUST override `apply_to_human`, even if just to immediately `return`.
