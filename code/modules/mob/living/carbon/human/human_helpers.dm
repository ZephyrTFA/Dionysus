...
/// Send player into a cryostasis pod. If with_paper = TRUE, place a paper with notification where the player was.
/mob/living/carbon/human/proc/send_to_cryo(with_paper = FALSE)
	//effect
	...
	if(with_paper)
		...
		cryo_paper.info = "Our sincerest apologies, [name][job ? ", [job]," : ""] had to be sent back into Cryogenic Storage for reasons that cannot be elaborated on at the moment.<br><br>Sincerely,<br><i>Dionysus Anti-Sudden Sleep Disorder Agency</i>"
		cryo_paper.update_appearance()
	//find cryostasis pod
	for(var/obj/machinery/cryostasis_pod/cryo in GLOB.valid_cryopods)
